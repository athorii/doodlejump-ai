...
std::vector<GameObject*> SequenceManager::Sequence::toObject() const {
    ...
    for (auto& e : entities) {
        ...
        case GameObject::CollisionType::spring:
            ...
            break;
        case GameObject::CollisionType::hat:
            width = sprite::item[sprite::Item::HAT].w;
            height = sprite::item[sprite::Item::HAT].h;
            seqObjs.push_back(new Hat(RectangleF(getTopLeftFromBottomMiddle(e.x, e.y, width, height), width, height)));
            break;
        ...
        case GameObject::CollisionType::enemySoucoupe:
            ...
            seqObjs.push_back(new EnemySoucoupe(RectangleF(getTopLeftFromBottomMiddle(e.x, e.y, width, height), width, height),
                                                RectangleF(10, 10, width - 20, height - 20)));
            break;
        case GameObject::CollisionType::enemyHole:
            width = sprite::enemy[sprite::Enemy::HOLE].w;
            height = sprite::enemy[sprite::Enemy::HOLE].h;
            seqObjs.push_back(new EnemyHole(RectangleF(getTopLeftFromBottomMiddle(e.x, e.y, width, height), width, height),
                                            RectangleF(10, 10, width - 20, height - 20)));
            break;
        default: break;
        }
